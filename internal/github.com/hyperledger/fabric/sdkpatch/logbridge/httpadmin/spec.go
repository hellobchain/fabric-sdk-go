...
func (h *SpecHandler) ServeHTTP(resp http2.ResponseWriter, req *http2.Request) {
	switch req.Method {
	case http2.MethodPut:
		var logSpec LogSpec
		decoder := json.NewDecoder(req.Body)
		if err := decoder.Decode(&logSpec); err != nil {
			h.sendResponse(resp, http2.StatusBadRequest, err)
			return
		}
		req.Body.Close()
		...
		resp.WriteHeader(http2.StatusNoContent)

	case http2.MethodGet:
		h.sendResponse(resp, http2.StatusOK, &LogSpec{Spec: h.Logging.Spec()})
...
func (h *SpecHandler) sendResponse(resp http2.ResponseWriter, code int, payload interface{}) {
