...
func isSM2CertBytes(certByte []byte) (bool, error) {
	cert, err := x509.ParseCertificate(certByte)
	if err != nil {
		return false, fmt.Errorf("ParseCertificate error: %v", err)
	}
	return isSM2SignedCert(cert), nil

}

func IsSM2CertPEM(idBytes []byte) (bool, error) {
	if idBytes == nil {
		return false, errors.New("GetCertFromPem error: nil idBytes")
	}
	// Decode the pem bytes
	pemCert, _ := pem.Decode(idBytes)
	if pemCert == nil {
		return false, errors.Errorf("GetCertFromPem error: could not decode pem bytes [%v]", idBytes)
	}
	return isSM2CertBytes(pemCert.Bytes)
}
...
func sanitizeECDSASignedCert(cert *x509.Certificate, parentCert *x509.Certificate) (*x509.Certificate, error) {
	...
	if isSM2SignedCert(cert) { // SM2证书的S不需要任何调整
		return cert, nil
	} else {
