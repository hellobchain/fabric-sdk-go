...
func (msp *bccspmsp) getCertifiersIdentifier(certRaw []byte) ([]byte, error) {
	...
	var chain []*x5092.Certificate
	if root {
		chain = []*x5092.Certificate{cert}
	} else {
...
func (msp *bccspmsp) setupCAs(conf *m.FabricMSPConfig) error {
	...
	msp.opts = &x5092.VerifyOptions{Roots: x5092.NewCertPool(), Intermediates: x5092.NewCertPool()}
...
func (msp *bccspmsp) setupCRLs(conf *m.FabricMSPConfig) error {
	...
	for i, crlbytes := range conf.RevocationList {
		crl, err := x5092.ParseCRL(crlbytes)
		if err != nil {
...
func (msp *bccspmsp) setupTLSCAs(conf *m.FabricMSPConfig) error {
	...
	opts := &x5092.VerifyOptions{Roots: x5092.NewCertPool(), Intermediates: x5092.NewCertPool()}

	// Load TLS root and intermediate CA identities
	msp.tlsRootCerts = make([][]byte, len(conf.TlsRootCerts))
	rootCerts := make([]*x5092.Certificate, len(conf.TlsRootCerts))
	...
	intermediateCerts := make([]*x5092.Certificate, len(conf.TlsIntermediateCerts))
	...
	// ensure that our CAs are properly formed and that they are valid
	for _, cert := range append(append([]*x5092.Certificate{}, rootCerts...), intermediateCerts...) {
		if cert == nil {
			continue
		}
