...
func CreateToken(csp core.CryptoSuite, cert []byte, key core.Key, method, uri string, body []byte) (string, error) {
	...
	var token string
	var hash core.HashOpts
	switch puk := publicKey.(type) {
	case *ecdsa.PublicKey:
		if ecdsa.IsSM2(puk.Params()) {
			hash = factory.GetSM3Opts()
		} else {
			hash = factory.GetSHAOpts()
		}
		token, err = GenToken(csp, cert, key, method, uri, body, hash)
		if err != nil {
			return "", err
		}
	}
...
//GenToken signs the http body and cert with ECDSA using EC private key
func GenToken(csp core.CryptoSuite, cert []byte, key core.Key, method, uri string, body []byte, hash core.HashOpts) (string, error) {
	b64body := B64Encode(body)
	b64cert := B64Encode(cert)
	b64uri := B64Encode([]byte(uri))
	payload := method + "." + b64uri + "." + b64body + "." + b64cert

	return genToken(csp, key, b64cert, payload, hash)
}

func genToken(csp core.CryptoSuite, key core.Key, b64cert, payload string, hash core.HashOpts) (string, error) {
	digest, digestError := csp.Hash([]byte(payload), hash)
	if digestError != nil {
...
func HTTPRequestToString(req *http2.Request) string {
...
func HTTPResponseToString(resp *http2.Response) string {
