...
type conn struct {
	...
	// nil means not TLS.
	tlsState *gmtls.ConnectionState
...
func (c *conn) serve(ctx context.Context) {
	...
	if tlsConn, ok := c.rwc.(*gmtls.Conn); ok {
		if d := c.server.ReadTimeout; d != 0 {
		...
		if err := tlsConn.Handshake(); err != nil {
			...
			if re, ok := err.(gmtls.RecordHeaderError); ok && re.Conn != nil && tlsRecordHeaderLooksLikeHTTP(re.RecordHeader) {
		...
		c.tlsState = new(gmtls.ConnectionState)
...
type Server struct {
	...
	// automatically.
	TLSNextProto map[string]func(*Server, *gmtls.Conn, Handler)
...
func (srv *Server) ServeTLS(l net.Listener, certFile, keyFile string) error {
	...
	if !configHasCert || certFile != "" || keyFile != "" {
		var err error
		config.Certificates = make([]gmtls.Certificate, 1)
		config.Certificates[0], err = gmtls.LoadX509KeyPair(certFile, keyFile)
		if err != nil {
			return err
		}
	}

	tlsListener := gmtls.NewListener(l, config)
