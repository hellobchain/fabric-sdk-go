...
import (
	...
	"errors"
	"fmt"
	"github.com/hyperledger/fabric-sdk-go/third_party/smalgo/gmhttp/httptrace"
...
type Transport struct {
	...
	// automatically.
	TLSNextProto map[string]func(authority string, c *gmtls.Conn) RoundTripper
...
func (t *Transport) Clone() *Transport {
	...
	if !t.tlsNextProtoWasNil {
		npm := map[string]func(authority string, c *gmtls.Conn) RoundTripper{}
...
func (pconn *persistConn) addTLS(name string, trace *httptrace.ClientTrace) error {
	...
	if err := <-errc; err != nil {
		...
		if trace != nil && trace.TLSHandshakeDone != nil {
			trace.TLSHandshakeDone(gmtls.ConnectionState{}, err)
		}
		return err
	}
...
func (t *Transport) dialConn(ctx context.Context, cm connectMethod) (pconn *persistConn, err error) {
	...
	if cm.scheme() == "https" && t.DialTLS != nil {
		...
		if tc, ok := pconn.conn.(*gmtls.Conn); ok {
	...
	if s := pconn.tlsState; s != nil && s.NegotiatedProtocolIsMutual && s.NegotiatedProtocol != "" {
		if next, ok := t.TLSNextProto[s.NegotiatedProtocol]; ok {
			return &persistConn{t: t, cacheKey: pconn.cacheKey, alt: next(cm.targetAddr, pconn.conn.(*gmtls.Conn))}, nil
		}
	}
