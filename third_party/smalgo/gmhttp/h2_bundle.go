...
// c is never closed.
func (p *http2clientConnPool) addConnIfNeeded(key string, t *http2Transport, c *gmtls.Conn) (used bool, err error) {
	p.mu.Lock()
...
func (c *http2addConnCall) run(t *http2Transport, key string, tc *gmtls.Conn) {
...
type http2connectionStater interface {
	ConnectionState() gmtls.ConnectionState
}
...
func http2ConfigureServer(s *Server, conf *http2Server) error {
	...
	if s.TLSConfig == nil {
		...
		for i, cs := range s.TLSConfig.CipherSuites {
			switch cs {
			case gmtls.TLS_ECDHE_RSA_WITH_AES_128_GCM_SHA256,
	...
	if s.TLSNextProto == nil {
		s.TLSNextProto = map[string]func(*Server, *gmtls.Conn, Handler){}
	}
	protoHandler := func(hs *Server, c *gmtls.Conn, h Handler) {
...
func (s *http2Server) ServeConn(c net.Conn, opts *http2ServeConnOpts) {
	...
	if tc, ok := c.(http2connectionStater); ok {
		sc.tlsState = new(gmtls.ConnectionState)
		*sc.tlsState = tc.ConnectionState()
		...
		if sc.tlsState.Version < gmtls.VersionTLS12 {
...
type http2serverConn struct {
	...
	tlsState         *gmtls.ConnectionState      // shared by all handlers, like net/http
...
func (sc *http2serverConn) newWriterAndRequestNoBody(st *http2stream, rp http2requestParam) (*http2responseWriter, *Request, error) {
	...
	var tlsState *gmtls.ConnectionState // nil if not scheme https
	if rp.scheme == "https" {
...
type http2Transport struct {
	...
	DialTLS func(network, addr string, cfg *gmtls.Config) (net.Conn, error)
...
func http2configureTransport(t1 *Transport) (*http2Transport, error) {
	...
	upgradeFn := func(authority string, c *gmtls.Conn) RoundTripper {
	...
	if m := t1.TLSNextProto; len(m) == 0 {
		t1.TLSNextProto = map[string]func(string, *gmtls.Conn) RoundTripper{
			"h2": upgradeFn,
		}
	} else {
		m["h2"] = upgradeFn
...
type http2ClientConn struct {
	...
	tlsState  *gmtls.ConnectionState // nil only for specialized impls
...
func (t *http2Transport) newTLSConfig(host string) *gmtls.Config {
	cfg := new(gmtls.Config)
...
func (t *http2Transport) dialTLS() func(string, string, *gmtls.Config) (net.Conn, error) {
	if t.DialTLS != nil {
		return t.DialTLS
	}
...
func (t *http2Transport) dialTLSDefault(network, addr string, cfg *gmtls.Config) (net.Conn, error) {
	cn, err := gmtls.Dial(network, addr, cfg)
	if err != nil {
		return nil, err
	}
