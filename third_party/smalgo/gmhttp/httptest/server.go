...
import (
	...
	"github.com/hyperledger/fabric-sdk-go/third_party/smalgo/gmhttp/internal"
...
func (s *Server) StartTLS() {
	...
	cert, err := gmtls.X509KeyPair(internal.LocalhostCert, internal.LocalhostKey)
	if err != nil {
	...
	if existingConfig != nil {
		...
		s.TLS = new(gmtls.Config)
	}
	if s.TLS.NextProtos == nil {
		s.TLS.NextProtos = []string{"http/1.1"}
	}
	if len(s.TLS.Certificates) == 0 {
		s.TLS.Certificates = []gmtls.Certificate{cert}
	}
	s.certificate, err = x5092.ParseCertificate(s.TLS.Certificates[0].Certificate[0])
	if err != nil {
		panic(fmt.Sprintf("httptest: NewTLSServer: %v", err))
	}
	certpool := x5092.NewCertPool()
	...
	s.Listener = gmtls.NewListener(s.Listener, s.TLS)
...
func (s *Server) Certificate() *x5092.Certificate {
