...
import (
	"errors"
	"fmt"
	gmtls2 "github.com/hyperledger/fabric-sdk-go/third_party/smalgo/gmtls"
...
func (c *tlsCreds) ClientHandshake(ctx context.Context, addr string, rawConn net.Conn) (_ net.Conn, _ credentials.AuthInfo, err error) {
	...
	conn := gmtls2.Client(rawConn, cfg)
...
func (c *tlsCreds) ServerHandshake(rawConn net.Conn) (net.Conn, credentials.AuthInfo, error) {
	conn := gmtls2.Server(rawConn, c.config)
...
func NewTLS(c *gmtls2.Config) credentials.TransportCredentials {
...
func NewClientTLSFromCert(cp *x509.CertPool, serverNameOverride string) credentials.TransportCredentials {
	return NewTLS(&gmtls2.Config{ServerName: serverNameOverride, RootCAs: cp})
}
...
func NewClientTLSFromFile(certFile, serverNameOverride string) (credentials.TransportCredentials, error) {
	...
	return NewTLS(&gmtls2.Config{ServerName: serverNameOverride, RootCAs: cp}), nil
}

// NewServerTLSFromCert constructs TLS credentials from the input certificate for server.
func NewServerTLSFromCert(cert *gmtls2.Certificate) credentials.TransportCredentials {
	return NewTLS(&gmtls2.Config{Certificates: []gmtls2.Certificate{*cert}})
}

// NewServerTLSFromFile constructs TLS credentials from the input certificate file and key
// file for server.
func NewServerTLSFromFile(certFile, keyFile string) (credentials.TransportCredentials, error) {
	cert, err := gmtls2.LoadX509KeyPair(certFile, keyFile)
	if err != nil {
		return nil, err
	}
	return NewTLS(&gmtls2.Config{Certificates: []gmtls2.Certificate{cert}}), nil
}
