...
import (
	...
	"github.com/hyperledger/fabric-sdk-go/third_party/smalgo/gmtls/gmcredentials/echo"
...
func serverRun() {
	cert, err := gmtls2.LoadX509KeyPair(ca, cakey)
	if err != nil {
		log.Fatal(err)
	}
	...
	creds := NewTLS(&gmtls2.Config{
		ClientAuth:   gmtls2.RequireAndVerifyClientCert,
		Certificates: []gmtls2.Certificate{cert},
		ClientCAs:    certPool,
	})
...
func clientRun() {
	cert, err := gmtls2.LoadX509KeyPair(admin, adminkey)
	if err != nil {
		log.Fatal(err)
	}
	certPool := x509.NewCertPool()
