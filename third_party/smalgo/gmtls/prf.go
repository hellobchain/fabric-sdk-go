...
func prfAndHashForVersion(version uint16, suite *cipherSuite) (func(result, secret, label, seed []byte), x509.Hash) {
	switch version {
	case VersionSSL30:
		if suite.flags&suiteSM2 != 0 {
			return prf12(sm3.New), x509.SM3
		}
		return prf30, x509.Hash(0)
	case VersionTLS10, VersionTLS11:
		if suite.flags&suiteSM2 != 0 {
			return prf12(sm3.New), x509.SM3
		}
		return prf10, x509.Hash(0)
	case VersionTLS12:
		if suite.flags&suiteSM2 != 0 {
			return prf12(sm3.New), x509.SM3
		}
		return prf12(sha256.New), x509.SHA256
	default:
...
// TLS hash identifier.
func lookupTLSHash(hash uint8) (x509.Hash, error) {
	switch hash {
	case hashSHA1:
...
func (h finishedHash) hashForClientCertificate(signatureAndHash signatureAndHash, masterSecret []byte) ([]byte, x509.Hash, error) {
	...
	if h.version == VersionSSL30 {
		...
		return finishedSum30(md5Hash, sha1Hash, masterSecret, nil), x509.MD5SHA1, nil
	}
	if h.version >= VersionTLS12 {
	...
	if signatureAndHash.signature == signatureSM2 {
		return h.server.Sum(nil), x509.SM3, nil
	}
	if signatureAndHash.signature == signatureECDSA {
		return h.server.Sum(nil), x509.SHA1, nil
	}

	return h.Sum(), x509.MD5SHA1, nil
}
