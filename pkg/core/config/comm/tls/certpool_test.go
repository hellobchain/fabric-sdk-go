...
var goodCert = &x5092.Certificate{
...
func verifyCertPoolInstance(t *testing.T, pool *x5092.CertPool, fabPool CertPool, numberOfCertsInPool, numberOfCerts, numberOfCertsByName, numberOfSubjects int, dirty int32) {
...
func createNCerts(n int) []*x5092.Certificate {
	var certs []*x5092.Certificate
	for i := 0; i < n; i++ {
		cert := &x5092.Certificate{
...
func getCertFromPEMBytes(pemCerts []byte) (*x5092.Certificate, error) {
	for len(pemCerts) > 0 {
		var block *pem.Block
